...
class API
{
    ...
    protected $wssapi = 'wss://api.hitbtc.com'; // /< REST endpoint for the withdrawals
    protected $stream = 'wss://st.hitbtc.com/'; // /< Endpoint for establishing websocket connections
    ...
    public function tt()
    {
        ...
        $parameters['params']['algo'] = 'BASIC';
        $parameters['params']['pKey'] = 'e47d46f031b588296246c7ff3d6c244c';
        $parameters['params']['sKey'] = 'bc300c4e1e9a670fadd69228cd08a56e';
        
        return $this->httpRequest("/api/2/ws", "GET", $parameters, true);
    }
    ...
    private function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, "User-Agent: Mozilla/4.0 (compatible; PHP Okayjava API)");
        // Post and postfields
        if ($method === "POST") {
...
//         }
    ...
    public function auth(callable $callback)
    {
        ...
        // @codeCoverageIgnoreStart
        // phpunit can't cover async function
        \Ratchet\Client\connect($this->wssapi . '/api/2/ws')->then(function ($ws) use ($callback, $endpoint) {
            $ws->on('message', function ($data) use ($ws, $callback, $endpoint) {
